...
def _normalize_dimension_text(text: str, aggressive: bool = True) -> str:
    ...
    # Rewrite compact hyphen-separated fractions without explicit slash:
    #   8 - 1516 -> 8 - 15/16
    #   3-716    -> 3-7/16
    # Keep this numeric-left gated to avoid touching IDs like A-1516.
    def _mixed_tail_no_slash_repl(m):
        whole = m.group(1)
        compact = m.group(2)
        expanded = _expand_compact_fraction_digits(
            compact,
        ...
        if not expanded:
            return m.group(0)
        if " " in expanded:
            # Do not rewrite into mixed-number forms in no-slash mode.
            return m.group(0)
        spacer = " " if (" " in m.group(0)) else ""
        return f"{whole}{spacer}-{spacer}{expanded}"

    t = re.sub(
        r"(?<!\d)(\d+)\s*-\s*([0-9]{3,6})(?=(?:\s*[^\d/]|$))",
        _mixed_tail_no_slash_repl,
        t,
    )
